...
from . import faceutils as futils
...
def get_face_parsing(x,face_paseing_model):
...
def load_data_from_image(non_makeup_img, makeup_img,opts,face_paseing_model):
    ...
    non_makeup_parse = get_face_parsing(non_makeup_img,face_paseing_model)
    non_makeup_parse = cv2.resize(non_makeup_parse, (opts.resize_size, opts.resize_size),interpolation=cv2.INTER_NEAREST)
    makeup_parse = get_face_parsing(makeup_img,face_paseing_model)
...
def makeup_transfer256(non_makeup_image, makeup_image,makeup_model,opts,face_paseing_model):
    data=load_data_from_image(non_makeup_image, makeup_image, opts=opts,face_paseing_model=face_paseing_model)
    with torch.no_grad():
